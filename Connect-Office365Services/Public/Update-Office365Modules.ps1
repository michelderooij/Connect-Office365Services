function Update-Office365Modules {
    ...
    if ( -not $local:IsAdmin) {
        Write-Warning 'Running non-elevated: modules installed in AllUsers scope cannot be upgraded and will be skipped.'
    }
    ...
    foreach ( $local:Item in $local:Functions) {
        ...
        if ( ($local:Module).RepositorySourceLocation) {
            ...
            if ( -not $local:IsAdmin -and ($local:Module.ModuleBase -like "$local:ProgramFilesPath*")) {
                Write-Host ('{0}: Skipped' -f $local:Item.Description) -ForegroundColor $script:myConsoleColors.Warning
                continue
            }
            ...
            if ( $local:NewerAvailable) {
                ...
                if ( $local:UpdateSuccess) {
                    ...
                    # If we just updated ourselves, the new version is now on disk but the old
                    # in-memory code is still executing. Deleting the old module folder would
                    # break the running session, so skip the uninstall step and ask the user
                    # to restart PowerShell to load the new version.
                    if ( $local:Item.Module -eq $MyInvocation.MyCommand.Module.Name) {
                        Write-Warning ('{0} was updated to v{1}. Restart PowerShell to load the new version.' -f $local:Item.Module, $local:OnlineVer)
                        continue
                    }
