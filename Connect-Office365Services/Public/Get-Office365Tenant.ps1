function Get-Office365Tenant {
    If( $script:myOffice365Services['Office365Credential']) {
        $local:OpenIdInfo= Invoke-RestMethod ('https://login.windows.net/{0}/.well-known/openid-configuration' -f ($script:myOffice365Services['Office365Credential'].UserName.Split('@')[1])) -Method GET
